	...
	switch (mEstados){
		...
		case SI_OFF_READ_0:
			if (cameraCLKBit_GetVal()){
				cameraSIBit_ClrVal();
				while(cameraSIBit_GetVal());
				//AD2_MeasureChan(0,0);// Liga a medida do conversor AD
				AD2_Measure(0);
				contClocks++;
...
{
	if (contClocks < 129)
		//	AD2_GetChanValue8(0,&(vetor_Camera[togBuffer][contClocks-1]));
		AD2_GetValue8(&(vetor_Camera[togBuffer][contClocks-1]));
}
